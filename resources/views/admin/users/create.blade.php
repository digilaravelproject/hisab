    ...
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('admin.users.index') }}"
            class="w-9 h-9 bg-white border border-[#E5EAF2] rounded-lg flex items-center justify-center
              text-gray-500 no-underline hover:bg-[#F0F4F8] hover:text-navy transition-all">
            ←
        </a>
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-navy tracking-tight">Create New User</h1>
            ...
            <div class="lg:col-span-2 flex flex-col gap-5">
                ...
                <div class="bg-white border border-[#E5EAF2] rounded-2xl overflow-hidden">
                    ...
                    <div class="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        {{-- Reminder Time --}}
                        <div>
                            <label class="block text-[13px] font-semibold text-navy mb-2">Daily Reminder Time</label>
                            <input type="time" name="reminder_time" value="{{ old('reminder_time', '22:00') }}"
                                class="w-full px-4 py-2.5 border border-[#E5EAF2] rounded-xl text-[13.5px]
                                      font-mono text-navy outline-none transition-all bg-[#F8FAFC]
                                      focus:border-navy focus:bg-white">
                        </div>
                ...
                <div class="bg-white border border-[#E5EAF2] rounded-2xl overflow-hidden">
                    <div class="px-5 py-4 border-b border-[#E5EAF2]">
                        ...
                        <p class="text-[11px] text-gray-400 mt-1">Select one or more roles for this user</p>
                    </div>
                    <div class="p-5 grid grid-cols-2 sm:grid-cols-3 gap-3">
                        @foreach ([['value' => 'employee', 'label' => '👔 Employee'], ['value' => 'farmer', 'label' => '🌾 Farmer'], ['value' => 'shopkeeper', 'label' => '🏪 Shopkeeper'], ['value' => 'proprietor', 'label' => '💼 Proprietor'], ['value' => 'business_owner', 'label' => '🏢 Business Owner'], ['value' => 'transporter', 'label' => '🚛 Transporter']] as $role)
                            <label
                                class="flex items-center gap-3 p-3 border border-[#E5EAF2] rounded-xl
                                  cursor-pointer hover:border-navy hover:bg-[#EEF2FA] transition-all
                                  has-[:checked]:border-navy has-[:checked]:bg-[#EEF2FA]">
                                <input type="checkbox" name="user_types[]" value="{{ $role['value'] }}"
                                    {{ in_array($role['value'], old('user_types', [])) ? 'checked' : '' }}
                                    class="accent-navy w-4 h-4 shrink-0">
                ...
                <div class="bg-white border border-[#E5EAF2] rounded-2xl overflow-hidden">
                    ...
                    <div class="p-5 flex flex-col items-center gap-4">
                        <div id="photo-preview"
                            class="w-24 h-24 rounded-full bg-[#EEF2FA] flex items-center justify-center
                                text-3xl border-2 border-dashed border-[#C7D4EA]">
                            👤
                        </div>
