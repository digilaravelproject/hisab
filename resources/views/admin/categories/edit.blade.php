...
@section('content')
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-navy tracking-tight">Edit Category</h1>
            <p class="text-sm text-gray-500 mt-1">{{ $category->name }} · #{{ $category->id }}</p>
        </div>
    </div>
                    ...
                    <div class="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="type" required
                                ...
                                <option value="credit" {{ old('type', $category->type) === 'credit' ? 'selected' : '' }}>
                                    ↑ Credit (Income)
                                </option>
                                <option value="debit" {{ old('type', $category->type) === 'debit' ? 'selected' : '' }}>
                                    ↓ Debit (Expense)
                                </option>
                            </select>
                            @error('type')
                        ...
                        <div>
                            ...
                            <input type="text" name="icon" value="{{ old('icon', $category->icon) }}"
                                placeholder="e.g. 💰 🏠 🚗" maxlength="10" id="iconInput"
                ...
                <div class="bg-white border border-[#E5EAF2] rounded-2xl overflow-hidden">
                    ...
                    <div class="p-5 flex flex-col items-center gap-3">
                        <div class="w-16 h-16 rounded-2xl bg-[#EEF2FA] flex items-center justify-center text-3xl">
                            <span id="iconPreview">{{ old('icon', $category->icon) ?: '?' }}</span>
                        </div>
