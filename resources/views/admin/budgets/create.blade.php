...
@section('breadcrumb', 'Budgets / Create')
...
@section('content')
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-navy tracking-tight">Create Budget</h1>
            <p class="text-sm text-gray-500 mt-1">Set a budget target for a user and category.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.budgets.store') }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div class="bg-white border border-[#E5EAF2] rounded-2xl overflow-hidden">
                    ...
                    <div class="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <select name="category_id" required
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }}>{{ $category->name }} @if($category->type) ({{ ucfirst($category->type) }}) @endif</option>
                                @endforeach
                            </select>
                            @error('category_id')
                                <p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input type="number" name="target_amount" value="{{ old('target_amount') }}" step="0.01" min="0"
                        ...
                        <div>
                            ...
                            <select name="month" required
                                ...
                                @foreach(range(1, 12) as $month)
                                    <option value="{{ $month }}" {{ old('month') == $month ? 'selected' : '' }}>{{ \Carbon\Carbon::createFromDate(2000, $month, 1)->format('F') }}</option>
                                @endforeach
                            </select>
                            @error('month')
                                <p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>
            ...
            <div class="flex flex-col gap-5">
                <div class="bg-white border border-[#E5EAF2] rounded-2xl overflow-hidden">
                    <div class="px-5 py-4 border-b border-[#E5EAF2]">
                        <h3 class="text-sm font-semibold text-navy">Notes</h3>
                    </div>
                    <div class="p-5 text-[13px] text-gray-500 space-y-3">
                        <p>Each budget is linked to one user and one category for a specific month.</p>
                        <p>Duplicate budgets for the same user, category and month are blocked.</p>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-2">
                    <button type="submit"
                        class="w-full py-3 bg-navy text-white rounded-xl font-semibold text-[14px]
                               cursor-pointer border-none hover:bg-navy-dark transition-all font-[Sora]">Save Budget</button>
                    <a href="{{ route('admin.budgets.index') }}"
