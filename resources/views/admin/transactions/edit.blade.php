...
@section('content')
    <div class="min-h-screen bg-gray-50 py-8 px-4">
        <div class="max-w-3xl mx-auto">
            ...
            {{-- Header --}}
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Edit Transaction <span
                        class="text-gray-400">#{{ $transaction->id }}</span></h1>
                <p class="text-sm text-gray-500 mt-1">Update the details below to modify this transaction.</p>
            </div>
                ...
                <form method="POST" action="{{ route('admin.transactions.update', $transaction->id) }}">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select name="type" required
                                        ...
                                        <option value="credit" {{ $transaction->type === 'credit' ? 'selected' : '' }}>
                                            Credit (Income)</option>
                                        <option value="debit" {{ $transaction->type === 'debit' ? 'selected' : '' }}>Debit
                                            (Expense)</option>
                                    </select>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select name="source" required
                                        class="w-full appearance-none bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg px-4 py-2.5 pr-9 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                                        <option value="cash" {{ $transaction->source === 'cash' ? 'selected' : '' }}>Cash
                                        </option>
                                        <option value="bank" {{ $transaction->source === 'bank' ? 'selected' : '' }}>Bank
                                            Transfer</option>
                                        <option value="upi" {{ $transaction->source === 'upi' ? 'selected' : '' }}>UPI
                                        </option>
                                    </select>
                                    <div class="pointer-events-none absolute inset-y-0 right-3 flex items-center">
                                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor"
                    ...
                    <div class="px-6 py-4 border-b border-gray-100">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            {{-- Bank Account --}}
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">Bank Account</label>
                                <div class="relative">
                    ...
                    <div class="px-6 py-4 border-b border-gray-100">
                                    ...
                                    <select name="business_id"
                                        ...
                                        @foreach ($businesses as $business)
                                            <option value="{{ $business->id }}"
                                                {{ $transaction->business_id === $business->id ? 'selected' : '' }}>
                                                {{ $business->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mt-5">
                            ...
                            <textarea name="description" rows="3" placeholder="Optional notes..."
                                class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition resize-none">{{ $transaction->description ?? '' }}</textarea>
                        </div>
                    </div>
